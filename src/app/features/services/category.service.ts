...
export class CategoryService {
  ...
  getAll():Observable<ResponseModel<Category>>{
      return this.httpClient.get<ResponseModel<Category>>(
        'http://localhost:5278/api/Categories?PageIndex=0&PageSize=100'
      );
  }

  getById(id:number):Observable<Response<Category>>{
    return this.httpClient.get<Response<Category>>('http://localhost:5278/api/Categories/'+id)
  }
  ...
  deleteCategory(categoryId:number){
    return this.httpClient.delete('http://localhost:5278/api/Categories/'+categoryId);
  }
}
